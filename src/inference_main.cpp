...
int main(int argc, const char* argv[]) {
  ...
  int n_decomposed_denoiser=atoi(argv[5]);
  total_iter=atoi(argv[6]);	/* for several input */

  
  int n_all = n_alex + n_vgg + n_res + n_dense + n_wide + n_squeeze + n_mobile + n_mnasnet + n_inception + n_shuffle + n_resX + n_origin_denoiser + n_custom_denoiser + n_decomposed_denoiser;
  ...
  try {
	  	origin_denoiserModule = torch::jit::load("/home/kmsjames/very-big-storage/JSH/HGD/Guided-Denoise/GD_train/traced_origin_denoiser_2.pt", device);
	    origin_denoiserModule.to(device);

	  	custom_denoiserModule = torch::jit::load("/home/kmsjames/very-big-storage/JSH/HGD/Guided-Denoise/GD_train/traced_custom_denoiser_2.pt", device);
	    custom_denoiserModule.to(device);

	  	decomposed_denoiserModule = torch::jit::load("/home/kmsjames/very-big-storage/JSH/HGD/Guided-Denoise/GD_train/traced_tucker_denoiser.pt", device);
	    decomposed_denoiserModule.to(device);

      denseModule = torch::jit::load("/home/kmsjames/very-big-storage/inmo/pytorch-test-inmo/densenet_model.pt");
      denseModule.to(device);

    	resModule = torch::jit::load("/home/kmsjames/very-big-storage/inmo/pytorch-test-inmo/resnet_model.pt");
      resModule.to(device);

    	alexModule = torch::jit::load("/home/kmsjames/very-big-storage/inmo/pytorch-test-inmo/alexnet_model.pt");
      alexModule.to(device);
  
    	vggModule = torch::jit::load("/home/kmsjames/very-big-storage/inmo/pytorch-test-inmo/vgg_model.pt");
      vggModule.to(device);

    	wideModule = torch::jit::load("/home/kmsjames/very-big-storage/inmo/pytorch-test-inmo/wideresnet_model.pt");
      wideModule.to(device);
 
    	squeezeModule = torch::jit::load("/home/kmsjames/very-big-storage/inmo/pytorch-test-inmo/squeeze_model.pt");
      squeezeModule.to(device);

    	mobileModule = torch::jit::load("/home/kmsjames/very-big-storage/inmo/pytorch-test-inmo/mobilenet_model.pt");
      mobileModule.to(device);

    	mnasModule = torch::jit::load("/home/kmsjames/very-big-storage/inmo/pytorch-test-inmo/mnasnet_model.pt");
      mnasModule.to(device);

    	inceptionModule = torch::jit::load("/home/kmsjames/very-big-storage/inmo/pytorch-test-inmo/inception_model.pt");
      inceptionModule.to(device);

    	shuffleModule = torch::jit::load("/home/kmsjames/very-big-storage/inmo/pytorch-test-inmo/shuffle_model.pt");
      shuffleModule.to(device);

    	resXModule = torch::jit::load("/home/kmsjames/very-big-storage/inmo/pytorch-test-inmo/resnext_model.pt");
  ...
  if(n_inception || n_origin_denoiser || n_custom_denoiser || n_decomposed_denoiser){
  ...
  for(int i=0;i<n_reg;i++){
    ...
    net_input_reg[i].gap = net_input_reg[i].layers.size() - REG_FLATTEN;
    ...
    stream_index_L +=1;
#else
    net_input_reg[i].H_L = 0; 
    net_input_reg[i].index_s = stream_index_L;
    net_input_reg[i].priority = net_priority_L; // net priority_L는 0부터 올라간다
    stream_index_L+=1;
    net_priority_L-=1;
#endif
    for(int j=0;j<WARMING;j++){
      predict_regnet(&net_input_mnasnet[i]);
      net_input_reg[i].input = inputs;
    }
    std::cout << "====== END mnasnet WARMUP ======" << std::endl;
  }
